...
module.exports = function registerDb(program) {
  ...
  db.command('up')
    ...
    .option(
      '-f, --file <file>',
      'docker compose file path (default: auto-detect)'
    )
    .action(async (opts) => {
  ...
  db.command('logs')
    ...
    .argument(
      '<service>',
      'service name (e.g. neo4j, mongo, redis, cassandra1)'
    )
    .option(
      '-f, --file <file>',
      'docker compose file path (default: auto-detect)'
  ...
  db.command('seed-analytics')
    .description(
      'Run seedAnalyticsEvents.js (your existing Cassandra analytics seeder)'
    )
    .action(async () => {
      try {
